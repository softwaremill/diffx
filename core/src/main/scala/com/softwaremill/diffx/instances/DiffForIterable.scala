...
private[diffx] class DiffForIterable[T, C[W] <: Iterable[W]](
  ...
  override def apply(left: C[T], right: C[T], context: DiffContext): DiffResult = nullGuard(left, right) {
    (left, right) =>
      ...
      val matches = matchPairs(leftWithIndex, rightWithIndex, adjustedMatcher, List.empty)
  ...
  private def matchPairs(
    ...
    right match {
      case ::(rHead, tailRight) =>
        val maybeMatched = left.collectFirst { case l if matcher.isSameObject(rHead, l) => l -> rHead }
        maybeMatched match {
          case Some((lm, rm)) =>
            matchPairs(left.filterNot(l => l.key == lm.key), tailRight, matcher, matched :+ MatchResult.Matched(lm, rm))
          case None => matchPairs(left, tailRight, matcher, matched :+ MatchResult.UnmatchedRight(rHead))
        }
