 ...
 * </code>
 */
public final class DiffRowGenerator2 {
    ...
    private DiffRowGenerator2(Builder builder) {
    ...
    public ArrayList<List<AbstractDelta<String>>> generateDiffRows2(List<String> original, List<String> revised) {
        Patch<String> diff = DiffUtils.diff(original, revised, equalizer);
        List<AbstractDelta<String>> deltas = diff.getDeltas();
        ArrayList<List<AbstractDelta<String>>> abstractDeltas = new ArrayList<>();
        for (AbstractDelta<String> delta : deltas) {
            if(delta.getType() == DeltaType.CHANGE) {
                ...
                String[] sWords = source.split(" ");
                String[] wWords = target.split(" ");
                Patch<String> diff1 = DiffUtils.diffInline(source, target);
                abstractDeltas.add(diff1.getDeltas());
            }else{
                abstractDeltas.add(Arrays.asList(delta));
            }
        }
    ...
    private List<DiffRow> generateInlineDiffs(AbstractDelta<String> delta) {
        ...
        List<DiffRow> diffRows = new ArrayList<>();
        for (int j = 0; j < Math.max(origList.size(), revList.size()); j++) {
            var k = j;
            var dr = inlineDeltas.stream()
                    .filter(d -> d instanceof ChangeDelta)
                    .filter(d -> (d.getTarget().getPosition() <= k) && (k < (d.getTarget().getPosition() + d.getTarget().size())))
                    .findFirst()
                    .map(d-> new DiffRow(Tag.CHANGE, String.join("", d.getSource().getLines()), String.join("", d.getTarget().getLines())))
                    .orElseGet(() -> new DiffRow(Tag.EQUAL,origList.size() > k ? origList.get(k) : "",revList.size() > k ? revList.get(k) : ""));
            diffRows.
                    add(dr);
        }
        return diffRows;
    ...
    public static class Builder {
         ...
         */
        public DiffRowGenerator2 build() {
            return new DiffRowGenerator2(this);
        }
